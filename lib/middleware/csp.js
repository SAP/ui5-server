...
/**
 * Evaluates if the pathFragment is either part of the pathName or in the request header referer
 *
 ...
 */
function containsPathFragment(pathFragment, req, pathName) {
	return pathName.includes(pathFragment) ||
		(req.headers["referer"] && req.headers["referer"].includes(pathFragment));
}
 ...
 * @property {string[]} ignoredPathFragments uris which are ignored by the CSP reports, e.g. ["test-resources/sap/ui/qunit/testrunner.html"]
 */
...
function createMiddleware(sCspUrlParameterName, oConfig) {
	...
	router.use((req, res, next) => {
		...
		if (ignoredPathFragments.some((ignoredPathSegment)=>{
			return containsPathFragment(ignoredPathSegment, req, oParsedURL.pathname);
		})) {
			next();
			return;
		}
