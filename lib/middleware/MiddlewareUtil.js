...
class MiddlewareUtil {
	...
	/**
	 * @param {Function} onExitFunction function which gets executed on exit
	 */
	registerExitFunction(onExitFunction) {
		this.onExitFunctions.push(onExitFunction);
	}

	/**
	 * Executes all onExit functions in parallel
	 *
	 * @returns {Promise<void>} once all exitFunctions finished
	 */
	async executeExitFunctions() {
		await Promise.all(this.onExitFunctions.map((callback) => {
			return callback();
		}));
	}
}
