...
export function getSslCertificate(
	...
	return Promise.all([
		fileExists(keyPath).then(async (statsOrFalse) => {
			...
			if (statsOrFalse.mode & constants.S_IWUSR) {
				// We only check for "writable by owner" even though we are more interested in "readable by others"
				// This is because "On Windows, only S_IRUSR and S_IWUSR are available" according to Node.js docs
				log.verbose(`Private key file at ${keyPath} is writable, indicating outdated permissions. ` +
					`Fixing permissions...`);
		...
		fileExists(certPath).then(async (statsOrFalse) => {
			...
			if (statsOrFalse.mode & constants.S_IWUSR) {
				log.verbose(`Certificate at ${keyPath} is writable, indicating outdated permissions. ` +
					`Fixing permissions...`);
