 ...
 * @private
 */
async function _listen(app, port, changePortIfInUse, acceptRemoteConnections) {
	const options = {};
	...
	try {
		options.port = await findFreePort(port, portMax, host);
	} catch (err) {
		let error;
		if (changePortIfInUse) {
			error = new Error(
			...
			error = new Error(`EADDRINUSE: Port ${port} is already in use.`);
			error.code = "EADDRINUSE";
			error.errno = "EADDRINUSE";
			error.address = host;
			error.port = portMax;
