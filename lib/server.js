...
async function executeExitTasks(middlewareManager) {
	await middlewareManager.getMiddlewareUtil().executeExitFunctions();
}

function registerExitSigHooks(middlewareManager) {
	function createListener(exitCode) {
		return function() {
			// Asynchronously cleanup resources, then exit
			executeExitTasks(middlewareManager).then(() => {
...
module.exports = {
	...
	async serve(tree, {
		...
		registerExitSigHooks(middlewareManager);
