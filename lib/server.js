...
module.exports = {
	...
	async serve(tree, {
		...
		class CachedReader extends ui5Fs.AbstractReader {
			constructor(reader) {
				super();
				this._reader = reader;
				this._cache = {};
			...
			async _byPath(virPath, options, trace) {
				if (!process.env.UI5_SERVER_CACHE) {
					return this._reader._byPath(virPath, options, trace);
				}
				if (this._cache[virPath]) {
					console.log("Using from cache: " + virPath);
					return new ui5Fs.Resource(this._cache[virPath]);
				}
				const resource = await this._reader._byPath(virPath, options, trace);
				if (!resource) {
					return null;
				}
				console.log("Caching: " + virPath);
				this._cache[virPath] = {
					buffer: await resource.getBuffer(),
			...
			_byGlob(...args) {
				// console.log("_byGlob: " + args[0]);
				return this._reader._byGlob(...args);
			}
			_runGlob(...args) {
				return this._reader._runGlob(...args);
			}
		}

		const cachedSource = new CachedReader(projectResourceCollections.source);
		const cachedDependencies = new CachedReader(projectResourceCollections.dependencies);

		// TODO change to ReaderCollection once duplicates are sorted out
		const combo = new ReaderCollectionPrioritized({
			name: "server - prioritize workspace over dependencies",
			readers: [cachedSource, cachedDependencies]
		});

		const resources = {
			rootProject: cachedSource,
